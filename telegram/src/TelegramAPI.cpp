...
Telegram::Bot::Connector::Connector(std::string token) : m_token{std::move(token) }, m_request{std::make_shared<Types::HTTPrequest>("https://m_api.telegram.org/bot" + token) }, m_api{std::make_shared<Telegram::Bot::Types::API>("https://m_api.telegram.org/bot" + token, m_request)}
{
	JSONCPP_STRING err;
	Json::Value parsed;
...
{
	std::thread(&Telegram::Bot::Connector::update, this).detach();
	while (1) {
		m_block.lock();
...
void Telegram::Bot::Connector::update()
{
	while (1) {
		...
		std::string readBuffer;
		std::string aghl;
		aghl = "/getUpdates?m_offset=" + std::to_string(m_offset + 1);
